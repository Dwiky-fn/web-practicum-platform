...
import { MessageSquare, Send, X, AlertCircle, CheckCheck, Loader2, User } from "lucide-react"
import {
...
export default function LecturerChatDrawer({
  ...
  useEffect(() => {
    ...
    async function initLecturerChat() {
      try {
        setLoading(true)
        setError("")
  ...
  }, [isOpen, kelasPraktikumId, jobsheetId, studentId, user])
  ...
  useEffect(() => {
    if (!isOpen || !token || !activeConversation) return
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    if (!text || !activeConversation || !user || sending) return
    ...
    try {
      ...
      if (isWsConnected && chatSocketRef.current) {
        ...
        const sent = await sendChatMessage(activeConversation.id, {
          message: text,
