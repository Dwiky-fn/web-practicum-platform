...
export default function AdminClassDetailPage() {
  ...
  const filteredCandidates = useMemo(() => {
    ...
    if (!raw) return studentCandidates
    ...
    if (tokens.length === 0) return studentCandidates

    const matchedSet = new Set<string>()
    return studentCandidates.filter((student) => {
      const nim = (student.nim || "").toLowerCase().trim()
      const name = (student.fullname || "").toLowerCase().trim()

      const isMatch = tokens.some((token) => {
        const lowerToken = token.toLowerCase()
        return nim.includes(lowerToken) || name.includes(lowerToken)
      })
  ...
  return (
    <AdminLayout>
      ...
      {assignOpen && (
        <AdminModal
          title="Tempatkan Mahasiswa"
          description="Tambahkan mahasiswa ke kelas ini"
